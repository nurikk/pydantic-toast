...
- Python 3.13+ (as per pyproject.toml `requires-python = ">=3.13"`) + pydantic>=2.0.0 (TypeAdapter), asyncpg>=0.29.0 (PostgreSQL), redis>=5.0.0 (Redis), aiobotocore>=3.0.0 (S3) (004-type-adapter)
- PostgreSQL (JSONB), Redis (JSON strings), S3 (JSON objects) - via existing backends (004-type-adapter)
...
## Recent Changes
- 004-type-adapter: Added Python 3.13+ (as per pyproject.toml `requires-python = ">=3.13"`) + pydantic>=2.0.0 (TypeAdapter), asyncpg>=0.29.0 (PostgreSQL), redis>=5.0.0 (Redis), aiobotocore>=3.0.0 (S3)
